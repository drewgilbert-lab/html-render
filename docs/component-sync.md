...
**What "does not move the contract" means.** The stamped commit is the last commit that touched
what the contract is made of, not `HEAD`: `package.json`, `design-export.lock.json`,
`docs/authoring.md`, `src/`, `bin/`, and the two scripts that generate it. A commit to anything
else, a README fix, a workflow change, a note in these docs, leaves the generated bytes
identical and opens nothing.

It stamped `HEAD` for one day and that was wrong: every commit moved the contract, so the first
docs-only change after the pipeline went live opened a pull request and took the consumer's
plugin to 0.53.0 for a catalog that had not changed.
